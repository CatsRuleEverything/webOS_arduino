...
const char* subTopic = "espCam/img/get";
const char* topic_CONFIG = "JSONConfig";
const char* pubTopic = "espCam/img";
...
void take_picture() {
  ...
  if (MQTT_MAX_PACKET_SIZE == 128) {
    ...
    client.publish_P(pubTopic, fb->buf, fb->len, false);
  }
  else {
    //FAST MODE (increase MQTT_MAX_PACKET_SIZE)
    client.publish(pubTopic, fb->buf, fb->len, false);
